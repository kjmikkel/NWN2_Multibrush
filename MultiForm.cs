...
namespace Multibrush
{
	...
	public partial class MultiForm : Form
	{
		...
		private const int NUM_TEXTURE_TOTAL = 8;
		private const int NUM_TEXTURE_UNIQE = 6;
		...
		private textureData readTextureElement(XmlNode node) {
		...
		void exportBrush(object sender, EventArgs e) {
			try {
				...
				if (fileDia.ShowDialog() == DialogResult.OK) {
					...
					XmlDocument xmlDoc = new XmlDocument(); // XmlTextWriter(fileDia.FileName, System.Text.Encoding.UTF8);
					...
					int loopIndex = 1;
					foreach(textureData texture in textureList.Items) {
					...
					grass.SetAttribute("coverage", data.grassCoverage.ToString());
					grass.SetAttribute("density", data.grassDensity.ToString());
					grass.SetAttribute("bladeSize", data.grassSize.ToString());
					grass.SetAttribute("bladeSizeVariation", data.grassSizeVariation.ToString());
					grass.SetAttribute("grassOption", data.grassOption.ToString());
					
					int index = 1;
					foreach (string textureStr in grassTextures.getListOfNames()) {
					...
					brushSizeData.SetAttribute("inner", data.innerCircle.ToString());
					brushSizeData.SetAttribute("outer", data.outerCircle.ToString());
					rootNode.AppendChild(brushSizeData);
					
					XmlElement colourData = xmlDoc.CreateElement("Colour");
					
					colourData.InnerXml = data.col.ToArgb().ToString();
					colourData.SetAttribute("option", data.colourOption.ToString());
					colourData.SetAttribute("coverage", data.colourCoverage.ToString());
					colourData.SetAttribute("presure", data.colourPresure.ToString());
					rootNode.AppendChild(colourData);
					
					XmlElement randomData = xmlDoc.CreateElement("Random");
					randomData.SetAttribute("randomize", data.randomize.ToString());
